...
model = model_by_architecture("data1", num_epochs=epochs1, learning_rate=lr1, input_channels=channels, nmaps=nmaps,
                              order=order, exp_name="conv", nfilters=k)

accuracy_validation, loss_validation, loss_training, t_step = model.fit(train, val)

np.savez_compressed("vdata1-conv-metrics-1.npz", lval=loss_validation, ltrain=loss_training, t=t_step)
...
np.savez_compressed("vdata1-conv-metrics-2.npz", lval=loss_validation, ltrain=loss_training, t=t_step)
