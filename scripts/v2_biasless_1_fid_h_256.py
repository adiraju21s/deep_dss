...
def train_one_epoch(lr, noise_level, iteration):
    ...
    model = model_v2_biasless(
        exp_name="1-fid-h-256-{0}-{1}-{2}-{3}".format(noise_levels, ilr, decay_noise, decay_train), gc_depth=12,
    ...
    np.savez_compressed(
        "../metrics/v2-biaseless-1-fid-h-256-{0}-{1}-{2}-{3}-{4}-{5}.npz".format(noise_levels, ilr,
