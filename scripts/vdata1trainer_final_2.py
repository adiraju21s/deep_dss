...
def train_one_quartile_epoch(quartile, lr, iteration):
    ...
    model = model_by_architecture("data1", num_epochs=1, learning_rate=lr, input_channels=channels, nmaps=20,
                                  order=order, exp_name="final-noiseless", nfilters=k)
    ...
    np.savez_compressed("vdata1-final-noiseless-metrics-{0}-{1}.npz".format(iteration, quartile), lval=loss_validation,
