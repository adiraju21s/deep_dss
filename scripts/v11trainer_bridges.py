...
PATH_TO_CHECKPOINTS = "/pylon5/ch4s8kp/adiraj21/DeepSphere"
...
model = model_by_architecture("v11", path_to_checkpoints=PATH_TO_CHECKPOINTS, num_epochs=16, learning_rate=1e-4)
...
data = split_poisson_maps_by_dataset("Q1", path_to_output=PATH_TO_OUTPUT, scramble=True, density=0.4,
                                     deepsphere_dataset=True)
model = model_by_architecture("v11", path_to_checkpoints=PATH_TO_CHECKPOINTS, num_epochs=8, learning_rate=8e-5)
...
data = split_poisson_maps_by_dataset("Q1", path_to_output=PATH_TO_OUTPUT, scramble=True, density=0.2,
                                     deepsphere_dataset=True)
model = model_by_architecture("v11", path_to_checkpoints=PATH_TO_CHECKPOINTS, num_epochs=8, learning_rate=6e-5)
accuracy_validation, loss_validation, loss_training, t_step = model.fit(data, val, session=model._get_session())

data = split_poisson_maps_by_dataset("Q1", path_to_output=PATH_TO_OUTPUT, scramble=True,
                                     deepsphere_dataset=True)
model = model_by_architecture("v11", path_to_checkpoints=PATH_TO_CHECKPOINTS, num_epochs=8, learning_rate=2e-5)
accuracy_validation, loss_validation, loss_training, t_step = model.fit(data, val, session=model._get_session())
